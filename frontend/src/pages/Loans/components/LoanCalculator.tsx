...
function buildAmortization(principal: number, termMonths: number) {
  ...
  const data: { month: number; balance: number }[] = [
    { month: 0, balance: Math.round(balance) },
  ];
...
export function LoanCalculator({ amount, termMonths }: Props) {
  const { payment, totalInterest, data } = buildAmortization(
    amount,
    termMonths
  );
    ...
    <Section className="bg-brand-surface">
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-12">
        <StatCard
          ...
          value={fmtDecimal.format(payment)}
          accent
        />
        <StatCard
        ...
        <StatCard
          ...
          value={fmt.format(amount + totalInterest)}
        />
      </div>
...
function StatCard({
  ...
  return (
    ...
    >
      <p className="text-sm text-brand-fg-muted uppercase tracking-wider mb-2">
        {label}
      </p>
      <p
        className={`text-3xl font-bold ${accent ? "text-brand-accent" : "text-brand-fg"}`}
      >
        {value}
      </p>
    </div>
